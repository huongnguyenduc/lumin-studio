...
/** `prefers-reduced-motion` gate for the scroll-into-view calls below (always-must #4). SSR-safe: only
 *  called from event handlers/effects, never during render. */
 ...
 *  host-pinned finalUrl submitted as paymentProofUrl, plus a local `previewUrl` (an object URL over the
 *  just-picked File — the browser already has the bytes, no round-trip needed to show a thumbnail).
 *  `error` codes tell the shopper what to fix. */
type ProofState =
  | { status: 'idle' }
  | { status: 'uploading' }
  | { status: 'done'; finalUrl: string; fileName: string; previewUrl: string }
...
/**
 * Checkout (/thanh-toan — P2-d+f, merged onto ONE page). Guest-first: reads the persisted cart (useCart),
 * collects contact + shipping address, prices the cart server-side WITH the chosen province
 * (POST /price/quote → shipping + total, never summed client-side — conventions §Tiền / ADR-019), shows
 * the đổi-trả/PDPL disclosures (compliance §2/§3), the VietQR panel + receipt upload, and submits in one
 * shot: `POST /orders` fires only once the form validates, the total is settled, the engrave acks (if any)
 * are met, AND a receipt is uploaded — mirroring the storefront rule (QR → đính ảnh biên lai + xác nhận →
 * mới tạo đơn), just without a separate "continue" click in between (nothing in that rule requires two
 * screens; the QR is server-rendered from the saved STK regardless of cart total).
 *
 * States: mount skeleton (localStorage isn't readable during SSR) · config load error (retryable) · empty
 * cart (CTA to catalog) · the priced form · shipping/total loading + error inside the summary · no-STK
 * closed notice · uploading/uploaded/error receipt · submit error (incl. "chưa đính ảnh biên lai") · C2½
 * full-screen submitting · C3 wait-screen on success.
 */
export function CheckoutView({ config }: { config: CheckoutConfigResult }) {
  ...
  // Receipt upload progress, in-flight submit, the mapped submit error, and the created-order result
  // (which switches the view to the done screen). One page, one form (P2-d+f merged — no reason a
  // shopper who already sees the QR/upload panel below their address should have to click a SEPARATE
  // "continue" first; the server still gates order creation on a completed proof upload either way).
  const [proof, setProof] = useState<ProofState>({ status: 'idle' });
  const [submitting, setSubmitting] = useState(false);
  const [submitError, setSubmitError] = useState<
    'no_stk' | 'no_shipping_rule' | 'no_proof' | 'error' | null
  >(null);
  ...
  // friendly message instead of letting Garage 400 the oversize part. `previewUrl` is a LOCAL object URL
  // over the just-picked File — the browser already has the bytes, so the thumbnail needs no round-trip
  // (never the signed-GET admin uses for a private bucket — that's for viewing a STORED order later).
  const onPickFile = async (file: File | undefined) => {
    ...
    try {
      ...
      setProof((prev) => {
        if (prev.status === 'done') URL.revokeObjectURL(prev.previewUrl); // dropping the old thumbnail
        return {
          status: 'done',
          finalUrl,
          fileName: file.name,
  ...
  // Revoke the last preview object URL on unmount so a shopper who changes receipts several times, or
  // navigates away, doesn't leak blob: URLs for the session's lifetime.
  useEffect(() => {
    return () => {
      setProof((prev) => {
  ...
  // One-page submit (info + payment merged, C1+C2): validate the form, require a settled server total +
  // met engrave acks (same gates the old "continue" step had) + a completed receipt upload, THEN place the
  // order. The double-submit guard is the synchronous `submitLatch` ref (see its declaration) — it closes
  // the window that async setState leaves open. On 201 the cart is emptied (a placed order's items must
  // not linger) and the done view takes over; a mapped failure re-enables the form (latch reset) with a
  // loud message (never silent).
  const onSubmit = async (e: FormEvent) => {
    ...
    // transient error) — the summary already shows why (skeleton / no_shipping_rule / pricing error), the
    // button is disabled while pending, so this is only reachable via Enter — silent no-op.
    if (okQuote?.total === undefined) return;
    // ADR-012 dual-ack (checkout.go:241): an engraved cart can't submit until both boxes are ticked. The
    // button is already disabled while unmet; this guards the Enter-key path — silent no-op, the unchecked
    // required checkboxes are the visible nudge.
    if (!acksMet) return;
    if (proof.status !== 'done') {
      // The receipt upload has no disabled-button gate anymore (a one-page form with a dead submit
      // button reads as broken) — surface a loud, specific reason instead.
      setSubmitError('no_proof');
    ...
    setSubmitError(null);
    setSubmitting(true);
    // Carry the acks only for an engraved cart (both true, since acksMet just passed); the server ignores
    // them otherwise, so a non-engraved order omits them like a blank note.
    const validated: ValidatedCheckout = hasPersonalization
      ? { ...result.value, personalizationAck, engraveEchoConfirmed }
      : result.value;
    const res = await placeOrder(buildWebOrderInput(validated, items, proof.finalUrl));
    if (res.ok) {
  ...
  // Can't submit without a server-computed total for the chosen province (still quoting, unshippable, or
  // a transient quote error) — the summary shows why — nor while the ADR-012 engrave acks are unmet (a
  // non-engraved cart is never gated: acksMet is true). The proof upload has NO disabled-button gate (see
  // onSubmit) — a one-page form with a dead submit button reads as broken; it reports a loud reason instead.
  const submitDisabled = (provinceChosen && total === undefined) || !acksMet;
    ...
    <Shell heading={t('heading')} wide>
      ...
      >
        ...
        <div className="mt-5 flex min-w-0 flex-col gap-5 lg:col-start-1 lg:row-start-1 lg:mt-0">
          ...
          {stkConfigured ? (
            <>
              {/* VietQR panel — the server-built img.vietqr.io QR + the STK. The bank name is
                  intentionally not shown (config carries only the bin, not a human name); the QR image
                  renders it, and we show account number + holder. ponytail: bin→name map deferred
                  until the shop asks. */}
              <section className="rounded-lg border-2 border-border-strong bg-surface-card p-4">
                <h2 className="font-display text-base font-bold text-text-strong">
                  {t('payHeading')}
                </h2>
                <p className="mt-1 text-sm text-text-body">{t('payIntro')}</p>
                <div className="mt-3 flex items-center gap-4">
                  {/* Plain <img> for the remote QR host (matches product-detail / cart-line — no
                      next/image remotePatterns to maintain). */}
                  <img
                    src={vietqrUrl}
                    alt={t('qrAlt')}
                    width={160}
              ...
              {/* Receipt upload (P2-c). No disabled-button gate on it anymore — a live thumbnail once
                  done, and the submit button reports a specific error if it's still missing, rather
                  than sitting dead. */}
              <section className="rounded-lg border-2 border-border-strong bg-surface-card p-4">
                <h2 className="font-display text-base font-bold text-text-strong">
                  {t('proofHeading')}
                ...
                {proof.status === 'done' ? (
                  <div className="mt-3 flex items-center gap-3">
                    <img
                      ...
                      className="h-24 w-24 flex-none rounded-md border-2 border-border-subtle object-cover"
                    />
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold text-primary" aria-live="polite">
            ...
            <p
              role="alert"
              className="rounded-lg border-2 border-border-strong bg-surface-sunken p-6 text-center text-sm text-text-body"
            >
              {t('noStk')}
            </p>
          )}
          ...
          {submitError ? (
            <p ref={submitErrorRef} role="alert" className="text-sm font-semibold text-danger">
