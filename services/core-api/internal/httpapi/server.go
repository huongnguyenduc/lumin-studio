...
// internal/order + internal/money. The `queries` (sqlc.New(pool)) and `authVerifier`
// fields join when 3g/3e first consume them — added then to keep the unused-field gate
// (staticcheck) green in this foundation PR.
type Server struct {
	logger *slog.Logger
	pool   *pgxpool.Pool
	nats   NATSStatus
...
// exercise those dependencies (readiness then skips the corresponding check).
func NewServer(logger *slog.Logger, pool *pgxpool.Pool, nats NATSStatus) *Server {
	return &Server{logger: logger, pool: pool, nats: nats}
}
