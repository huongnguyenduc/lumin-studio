...
func newTestRouter() http.Handler {
	...
	return New(nil, a, nil, "luminstudio.vn", "test-gi") // routes under test never touch the pool
}
...
func TestPresignDisabledWithoutConfig(t *testing.T) {
	a := auth.New(config.Config{AdminPassword: "pw", JWTSecret: "t", JWTTTL: time.Hour})
	h := New(nil, a, nil, "luminstudio.vn", "test-gi")
...
func TestBearerGrantsMasterWrongTokenRejected(t *testing.T) {
	h := New(nil, auth.New(config.Config{AdminPassword: "pw", JWTSecret: "t", JWTTTL: time.Hour}), nil, "luminstudio.vn", "test-gi")
