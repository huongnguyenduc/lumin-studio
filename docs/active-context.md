...
## Focus
**➡️ P3-j-b COMMITTED `d960f41` → PR #74 open to `main`** (branch `feat/phase-3-admin-p3jb-model-upload`, off fresh `main` `94e7c05` = **P3-j-a #73 MERGED**; clean squash-tree match verified before rebranch). **ADR-036 = user chose Option A** (single presigned POST ≤100MB, NOT S3 multipart — refine ADR-005). **Core Done-gate Track 0/A/B/C COMPLETE**; Track D (Sản phẩm) BE both halves done. **P3-j 2-PR split DONE:** P3-j-a = catalog CRUD (#73 merged, `main` `94e7c05`) · **P3-j-b = model-upload infra + asset-jobs (#74 open, awaiting user merge + CI).** **NEXT after P3-j-b merges = P3-k** (FE `/san-pham` list) + **P3-l** (FE editor full + live-preview + wire the model-upload→asset-job flow built here).
