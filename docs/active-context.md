...
## Focus
**➡️ ADR-037 CONFIGURATOR EPIC — Stage 2a MERGED #76; 2b-1 (pricing = MONEY PATH) MERGED #77; 2b-2 (capture + wire) MERGED #78; Stage 2c (storefront consume — FE) BUILT+VERIFIED, reviews next.** P3-k **MERGED #75** (`main` `8a92edc`, local ff'd; stale P3-k branch left to prune). User expanded P3-l (FE editor) into the FULL design — all 4 rich features BUILT, not cut → **5-stage epic** (tasks #1-5). **Stage 1 (backbone design + ADR) COMPLETE:** ADR-037 in `decisions.md` (per-part colors + per-option choices = **additive jsonb capture**; **nullable `colors.part_id`**; **one-job-per-item** print; **3 pricing invariants**) + spec §02 updated (Part/OptionChoice entities + OrderItem partColors/optionChoices note), **oracle-reviewed + user-approved**. Plan `docs/plans/p3l-product-editor.md` (reframed → Stage 5).

**Stage ledger:** ①design+ADR ✅ · ②BE parts/choices ⏳ (2a/2b done, 2c FE building) · ③BE 3D-alignment · ④P3-s material-standards · ⑤FE editor (l-1/l-2, consumes 1-4).
**Stage 2 sub-PRs:** **2a** catalog model ✅ **MERGED #76** (`main` `fd144e3`) · **2b split (money path):** **2b-1** pricing (`Selection`+3 invariants+`PriceItem`, PURE) ✅ **MERGED #77** (`main` `c69222c`) · **2b-2** capture+wire ✅ **MERGED #78** (`main` `2c902b5`) · **2c** storefront consume (part-grouped colors+choice options+cart key) ✅ BUILT+VERIFIED (reviews next) · **2d** admin order-detail read (`ListOrderItems` jsonb + print-queue `part_colors`) ← NEXT.
