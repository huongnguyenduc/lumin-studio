...
**`3k` admin settings/STK ✅ BUILT · verify+integration(colima) green · guard 153 · adversarial review DONE + fixes applied ·
spec-guardian PASS (0 BLOCKER/0 WARN/2 NOTE) · chờ push→PR. (branch `feat/core-http-relay-3k` off `main` `3fb254e`.)**
...
## Next steps (1–3)
1. **Slice 3 · PR-3k — ✅ BUILT · verify+integration green · guard 153 · review DONE + fixes · spec-guardian PASS → COMMIT + push + PR
   NOW.** All gates green; commit the 7-file diff (3 new settings files + stubs.go + guard.test.sh + acceptance.md + active-context.md),
   push, open PR. **Flag in PR:** openapi `BankAccountUpdate` stays looser than server validation (server-authoritative fail-closed;
   add `pattern`/`minLength` là follow-up khi 3j consumes it). (**PR-3g already MERGED #28 — local `main` at `3fb254e`.**)
2. **Then remaining fan-out (parallel-safe):** **`3i` dashboard aggregates** (→ migration **000011**_dashboard_idx, since 3f
   took 000010; Asia/Ho_Chi_Minh "today" boundary; net-revenue formula) → **`3j`** admin dashboard frontend (needs 3i; the
...
| **Core slice 3 · PR-3k — admin settings/STK (`GET /admin/settings` + owner-only `PATCH /admin/settings/bank-account` + reply-template reads)** | **BUILT · verify+integration green · review DONE + fixes · spec-guardian PASS · chờ push→PR** | `feat/core-http-relay-3k` off `main` `3fb254e` (uncommitted) | `make verify-go` ✓ (golangci 0, sqlc vet+diff, oapi stale-check clean [no openapi change], `go test -race`) · **integration RAN vs real Postgres (colima, -race):** `TestUpdateBankAccountEndToEnd` (column+audit atomic, changed_by from ctx, GetSettings reflects) · `TestGetSettingsSeededDefaults` · `TestListReplyTemplatesEndToEnd` (ORDER BY title) · guard **153** (+1 settings ARM PROVEN binding ×3: `UpdateBankAccountTx` + `order.RoleOwner` + `actorFrom`; each mutate→152/1→restore) · **prior session left 3 duplicate stubs in stubs.go (compile blocker) → removed** · **authz-FIRST** (owner re-assert `order.RoleOwner`→403 defense-in-depth before body validation) + `cleanBankUpdate` **bin đúng 6 napas digits**/accountNumber digits ≤19 + `UpdateBankAccountTx` audit-on-commit + changed_by from actor ctx · **review wf_929df5a0-540 (5 money-out lenses × per-finding refute + critic, 12 agents): 3 raw → 0 confirmed / 3 refuted; critic 1 IMPORTANT (bin-6-digit) + 1 NOTE (authz-order) FIXED, 1 NOTE (append-only) REFUTED (2g covers)** · STK-01 acceptance Cụm 11 `[ ]` (Go-gated) · **no new deps · no new ADR** (impl conventions §57/ADR-012/domain-core RBAC) · **spec-guardian PASS 0/0/2 NOTE** (openapi looser than server = safe fail-closed; unreachable 422) |
| ADR-026 lane B/C/D · REC-20/28/39 | todo | — | — |

## Lần verify xanh gần nhất
**Core slice 3 · PR-3k — admin settings/STK (2026-07-02):** `make verify-go` rc=0 (gofmt + vet + golangci v2 **0** + sqlc
vet + sqlc diff + oapi generate+git-diff stale-check [clean — no openapi change] + `go test -race`) · guard.test.sh **153 / 0** ·
