...
> **Phase-1 sub-PR tally: 13/19 MERGED** (a,b,c,d,e,f,g,h,j,k,l,m,n; note this branch-copy of active-context lags — P1-m #45 is the real `origin/main` tip). Remaining: P1-i (360 media — degrade-only, sprite pipeline unconfirmed) · P1-o (guest lookup — **PR #46 OPEN, chờ merge**) · **P1-q (SEO — IN PROGRESS, this branch)** · P1-p (consent analytics) · P1-r/s (customer auth — BLOCKER-2: needs credentials migration + auth-mechanism decision, gated on user).
...
> **🔨 PR-P1-q (FE · SEO — server OG + Product/Offer JSON-LD + sitemap/robots/canonical) — BUILT · `pnpm verify` green (lint+typecheck+**111** test, 8 new) · `next build` COMPILES (sitemap graceful-degrade proven; `/` prerender fail = pre-existing P1-f core-api coupling, not a CI gate) · adversarial review `wf_08bde641` DONE (7 raw → 4 confirmed = 2 distinct issues, BOTH FIXED) · spec-guardian RUNNING · chờ PASS→push→PR.**
> (branch `feat/phase-1-storefront-p1q` off `origin/main` `25bb5c4` — **INDEPENDENT of the still-open P1-o #46**; expect a trivial `active-context.md`/`messages/vi.ts` rebase conflict when P1-o merges first.) User picked P1-q as next (2026-07-05). Consumes P1-c/P1-h (merged). **READ-ONLY** — ZERO order/payment/address/checkout (grep-clean).
...
> **NEXT:** user merge-gate. On merge: rebase-friendly (independent of P1-o #46). Remaining Phase-1 after P1-q: P1-i (degrade-only), P1-o (#46 open), P1-p (consent), P1-r/s (BLOCKER-2, user decision).
