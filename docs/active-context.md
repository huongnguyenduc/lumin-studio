...
## Focus
**➡️ P3-j-a BUILT + VERIFIED on `feat/phase-3-admin-p3j-product-write`** (off `main` `6383cd9` = P3-i #71 squash-merged). **Core Done-gate Track 0/A/B/C COMPLETE**; now on **Track D (Sản phẩm)**. **P3-j SPLIT into 2 PRs (Scope §1-PR-1-trục):** **P3-j-a = catalog write CRUD** (BE-only, THIS) + **P3-j-b = model-upload infra + asset-jobs + ADR-036** (NEXT, needs user's ADR call: single presigned-PUT-now vs full-multipart — minio-go can't presign multipart parts). **NEXT after P3-j-a = reviews → commit → user gate (push+PR)**, then P3-j-b, then P3-k (FE `/san-pham` list) + P3-l (FE editor full+live-preview).
