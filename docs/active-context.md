...
**`3h` transition endpoints ✅ BUILT · Docker-free + integration (colima) green · chờ push→PR. (branch
`feat/core-http-relay-3h` off `main` `7ab0159`.)** `POST /orders/{id}/transitions` — the RBAC-gated status-change endpoint.
...
## Next steps (1–3)
1. **Slice 3 · PR-3h — transition endpoints:** BUILT + Docker-free + integration(colima) green on `feat/core-http-relay-3h`
   (off `main` `7ab0159`). **Run full `make verify-go` + `guard.test.sh` (confirm guard 151) + spec-guardian, then push → PR**
   (owner merges). PR body: `POST /orders/{id}/transitions` dispatch-footgun (`to=PAID`→`ConfirmPaymentTx` only-emitter of
   `order.paid`, else→`AdvanceStatusTx`) + money-in owner-gate at boundary (staff reconcile→403 before tx; `ConfirmPaymentTx`
   hardcodes owner so domain guard can't) + SHIPPING `trackingCode` required (422) + `SetTrackingCodeTx` atomic w/ flip (no
   migration) + shared `toOrderDTO` assembler (3g reuses) + `errTrackingCodeRequired`→422; guard 151 (+1 transition ARM proven);
   PAY-01/SHP-01 `[ ]`; no new deps/ADR; ~150 non-test src.
2. **Remaining fan-out (all unblocked, parallel):** **`3g` checkout** (needs 3f ✓ — wires pricing/shipping/code/customer into
   `withTx`→`db.CreateOrderTx`; inbox staff-gated per critique BLOCKER; **reuses `dto.go` `assembleOrderDTO` from 3h**) · **`3i`
   dashboard** (→ **000011**_dashboard_idx, since 3f took 000010) · **`3k` settings/STK** → **`3j`** admin frontend (needs 3i).
   Handlers read the injected `Actor` from ctx (`actorFrom`) to build `order.TransitionContext`. Full DAG: `core-http-relay.md §1`.
3. **Housekeeping:** prune now-merged local branches `feat/core-http-relay`(3a)/`-3b`/`-3c-1`/`-3c-2`/`-3d`/`-3e-1`/`-3e-2`/`-3f`
   when chủ duyệt (all squash-merged → won't show under `git branch --merged`; verify by PR#, not sha). Harness follow-ups: the
   **testcontainers ARM** greps Go `_test.go` for `postgres.Run` unanchored → a `//`-commented boot call could false-pass (same
   comment-out class fixed for recipe ARMs); **decisions.md ADR-033 "migration 000008" aside now stale** (3f took 000010) — fix
   in a harness/doc round (hard-blocked file). Sau Core phase: ADR-026 lane B/C/D · REC-20/28/39.
