...
## Cụm 13 — Storefront catalog read (`docs/plans/phase-1-storefront.md` §2 · P1-a/P1-c/P1-d)
...
> classify `GetCategories` authPublic **+** `ListCategories` scope active-product tại SQL (`EXISTS ... status='active'` — non-leak, category chỉ-chứa-hàng-ẩn không rò) **+**
...
> **+** chính các test Go đó (unit Docker-free limiter/DTO/normalize/429 + integration vs PG thật). `[ ]` = "không do parser-TS gác".
