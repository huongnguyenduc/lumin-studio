...
- **`render-worker-gpu`** → `operations.md` §GPU + `asset-worker.md` (ràng buộc GTX-1060/VRAM khi task ops chạm render ngoài `services/asset-worker/**`).
- **`event-outbox`** → `domain-core.md` + ADR-006 (publish-on-commit/idempotency cho path admin-reconcile & checkout không auto-load domain-core).
...
`tests/harness/guard.test.sh` feed fixture vào từng hook và assert cổng chặn **fire** (rm -rf, sửa .env, hard-block decisions.md, test bị làm yếu, **special-casing REC-16**, loop detector…) + ca dương (lệnh lành đi qua) + mỗi rule có `paths:` + osm-mutation tồn tại/hợp-cú-pháp. Chạy **được pre-Phase-0** (không phụ thuộc app toolchain). **Wire vào CI khi `.claude/**` đổi** — vì mọi hook self-no-op tới Phase 0, "gate no-op" trông y hệt "gate pass"; self-test là cái phân biệt. Khi sửa hook, cập nhật fixture tương ứng.

## Mutation kill-gate OSM (`tests/harness/osm-mutation.test.sh`, REC-15/ADR-024 nháp)
Em ruột **deterministic** của `guard.test.sh` (KHÔNG phải LLM-judge) — chứng minh test OSM *ràng buộc* transition, không chỉ *pass* (coverage% mù với test vacuous; OSM là xương sống xuyên 4 surface ⇒ điểm mù cao nhất). **Self-check (luôn chạy):** áp bộ mutant cố định — `allow-all guard` · `swap from/to` (→ OSM-01) · `drop statusHistory` (→ OSM-02) · `drop reason-check` (→ OSM-03) — lên một toy-OSM pure-bash + toy-test, assert mỗi mutant bị **KILL** (test tương ứng chuyển đỏ). Mutant sống = test không ràng buộc → fail. **Real-arm:** tự kích khi Phase-0 land `packages/core` (áp cùng họ mutant lên OSM thật + chạy `order_state.*`); self-no-op khi OSM chưa có. Chạy ở **CI lane `.claude/**` cùng `guard.test.sh`** — **KHÔNG** wire vào inner-loop của `verify-before-stop` (giữ vòng agent nhẹ).
...
|---|---|---|
| `.claude/.skip-verify` | verify-before-stop | Bỏ qua green-gate một lần (ca môi trường) |
| `.claude/.allow-contract-edit` | guard-files | **Đường amend** file hợp đồng lõi: tạo → sửa (chỉ thêm ADR/Superseded) → xoá |
