...
| **t-2** | **NFC-encode print stage** | BE + Admin | t-1, P3-h | `ALTER TYPE print_stage ADD VALUE` NFC stage (nfc_tag only) · `AdvancePrintStage` handles it · kanban 5th column **conditional on nfc_tag** (`groupByStage`/`nextStage`/`print-board.tsx`) · encode action writes `chip_uid`+`encoded_at` → tag `ENCODED` (D2 mechanism) · empty/loading/error |
...
| **t-6** | **Analytics + PDPL finalize** | both | t-3, t-4 | Umami events (§10: `pettag_scanned`/`pet_activated`/`lostmode_toggled`/`finder_location_shared`/`pet_profile_edited`/`pet_theme_changed`) · `LostEvent` geo **retention** job · phone-mask helper · **skill `vn-compliance` pass** (consent log/replay at both points). May fold into t-3/t-4 |
