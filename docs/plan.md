...
> - [ ] `packages/core` land → `acceptance.ledger.test.ts` tồn tại + pass (ép **OSM-02 statusHistory** + **MNY-03 money một-formatter**).
...
> - [ ] `services/**/*.go` land → `Makefile` có **`verify-go`**; `*.rs` → **`verify-rs`**.
