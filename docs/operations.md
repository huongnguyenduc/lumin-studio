...
- **Storefront (Next) — `SITE_URL` (P1-q, bắt buộc):** origin công khai của storefront (scheme + host, **không** trailing slash, ví dụ `https://luminstudio.vn`) — server-only, **KHÔNG** `NEXT_PUBLIC_`. Feed canonical + sitemap + robots.txt + OG image tuyệt-đối (`apps/storefront/src/lib/site.ts`). **Thrown-not-defaulted** (như `CORE_API_URL`): localhost câm sẽ **de-index** site → thiếu env thì **mọi route + `next build` throw** (chạy qua `metadataBase` ở `layout.tsx`). **Phải include scheme** (`new URL()` cần `https://…`). Set trong `apps/storefront/.env.example` + deploy env khi container storefront land.
...
4. **`CUSTOMER_JWT_SECRET` (bắt buộc ở prod — PR-P1-r):** khoá ký HS256 cho session **khách storefront** (realm RIÊNG, ADR-030). **PHẢI khác `JWT_SECRET`** — cùng khoá thì token admin verify được như session khách và ngược lại. **Chưa set = core-api TỪ CHỐI khởi động** (fail-fast) vì dev-secret công khai → token khách **giả mạo được** → đọc lịch sử đơn của bất kỳ ai (rò PII). Cùng cờ `ALLOW_DEV_JWT_SECRET=true` mở cho local dev cả hai realm. **`CUSTOMER_JWT_TTL`** (default `720h` = 30 ngày; dài hơn admin để khách không bị đăng xuất giữa chừng). Khách **tự đăng ký** (`POST /customer/register`) — **không** có bước seed như owner.
5. **`TRACKING_SECRET` (bắt buộc ở prod — PR-P2-i):** khoá HMAC cho **token theo dõi đơn phone-less** (link C3 `/o/{code}-{token}`, D-P2-8). Token = `base64url(HMAC-SHA256(TRACKING_SECRET, mã-đơn))`, trả một lần ở `POST /orders` 201 và verify constant-time ở `GET /orders/track`. **Chưa set = core-api TỪ CHỐI khởi động** (fail-fast) vì dev-secret công khai → **ai cũng tính được token của đơn bất kỳ** → theo dõi timeline đơn người khác (BLOCKER-F). Cùng cờ `ALLOW_DEV_JWT_SECRET=true` mở cho local dev. **KHÔNG có cột lưu token** (recompute lúc đọc, không migration) → **xoay `TRACKING_SECRET` = vô hiệu MỌI link theo dõi cũ** (đây là cơ chế revoke hàng loạt duy nhất; per-order revoke là upgrade-path thêm cột `orders.tracking_token`, không thuộc Phase 2). **Ops:** token nằm trong URL query (`/orders/track?...&token=`, bearer secret) → cấu hình edge access-log (Caddy/Cloudflare) **KHÔNG giữ query-string** cho path `/orders/track` (khỏi ghi token vào log; review adversarial P2-i NOTE).
